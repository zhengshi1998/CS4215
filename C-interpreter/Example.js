...
arr_len:
    cmd => {
        // if is array literal, return its length
        if(cmd.expr.elems != null){
            push(S, cmd.expr.elems.length);
        } else if(cmd.expr.sym != null){
            // if is varibale, lookup in E and return its length
            const val = lookup(cmd.expr.sym, E);
            push(S, val.length);
        } else if(cmd.expr.fun != null) {
            push(A, cmd.expr);
            
            while (A.length != 0) {
                const cmd_temp = A.pop()
                if (microcode.hasOwnProperty(cmd_temp.tag)) {
                    microcode[cmd_temp.tag](cmd_temp)
                    //debug(cmd)
                } else {
                    error("", "unknown command: " + command_to_string(cmd_temp))
                }
            }
...
arr_lit: 
    cmd => {
        let ret = [];
        for(let i=cmd.elems.length-1;i>=0;i--){
            ret.add(cmd.elems[i].val);
        }
        
        // transfer the array literal into array vals and push it into stash
        push(S, ret);
    },
    // function f() { return [1,2,3,40]; } f().length;
